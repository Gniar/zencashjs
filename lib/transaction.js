...
function mkNullDataReplayScript(data, blockHeight, blockHash) {
  var dataHex = Buffer.from(data).toString('hex'); // Minimal encoding
  ...
  var blockHeightHex = blockHeightBuffer.toString('hex'); // Block hash is encoded in little indian
...
function mkPubkeyHashReplayScript(address, blockHeight, blockHash) {
  ...
  var subAddrHex = addrHex.substring(pubKeyHash.length, addrHex.length); // Minimal encoding
...
function serializeTx(txObj) {
  ...
  txObj.ins.map(function (i) {
    ...
    serializedTx += _buf16.toString('hex'); // Script Signature
